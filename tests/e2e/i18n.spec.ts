...
test.describe('Language switching', () => {
	test('the footer switcher moves the whole page to Italian and back', async ({ page }) => {
		...
		await expect(page).toHaveURL(new RegExp(`${BASE}/it-it/$`));
		...
		await expect(page).toHaveURL(new RegExp(`localhost:\\d+${BASE}/$`));
		await expect(page.locator('html')).toHaveAttribute('lang', 'en');
	});
	...
	test('switching keeps you on the page you were reading', async ({ page }) => {
		...
		await expect(page).toHaveURL(new RegExp(`${BASE}/it-it/collection/$`));
