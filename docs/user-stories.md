...
- **[done]** As a user on a browser without the File System Access API, I want the linking controls to be hidden rather than shown-and-broken, so that I am not offered a feature my browser cannot perform.
- **[planned]** As a user, I want to see a note in the DB modal explaining that file linking requires a compatible browser (Chrome 86+, Edge 86+, Safari 15.2+) and is not available in Firefox, so that I understand why the feature is absent rather than assuming it is missing by mistake. _(Today the controls are simply hidden; no explanatory note is rendered.)_
