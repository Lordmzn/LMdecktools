...
- **[done]** As a user, I want to see a note in the DB modal explaining which browsers support file linking, so that I understand why the feature is absent rather than assuming it is missing by mistake. _(#42 — the note sits next to Download copy, where the alternative to auto-save actually is. The note currently names "Safari 15.2+", which is wrong: Safari has never implemented the File System Access API on any platform. Correct text is desktop Chrome/Edge 86+ and Chrome Android 132+.)_
